...
function Game(roomInfo,pusher){
    ...
    this.runningInfo = {
        ...
        fleshArr:{              //动态表，每个环节都要刷新
            ...
            dayJobInfo:{targetsPos:[],infos:[]}      //信息类职业白天获得的信息
        },
        fleshInfo:{
            lastNgtOutInfoHtml:"",     //上一个夜晚出局的信息
            dawnGlobalInfoHtml:"",       //这个黎明的全局提示信息
            ngtAlnOut:false              //夜晚有外星人出局
        },
        listenArr:{             //流程相关监听队列
    ...
    this.initArrays = function(){
        ...
        for(var i = 0 ; i< readyList.length; i++){
            if( readyList[i]===true && userNickList[i]!= null && userNickList[i]!=undefined){  //这个位置有人，而且已经准备了
                ...
                //self.runningInfo.fleshArr.thisNightSkillBuffer.push(null); //TODO 应该不需要
    ...
    this.allocate = function(){
        //基本局测试执行：[][][]为测试
        var allocateData = new alloctInfo(self.runningInfo.ttlPlyrNum,[],[],[]);
        var allocateRes = alloctManager.getAllocRes(allocateData);
        allocateRes = { partyList: [ 2, 2, 2, 1, 1, 1, 1, 1 ],
            jobList: [ 0, 0, 0, 0, 5, 9, 10, 18 ],
            HmnNum: 5,
            GstNum: 3,
            alnNum: 0 }
        console.log(allocateRes)//TODO
        var initWordsRes = wordsManager.getWordsWithoutArgs();
        self.runningInfo.quickArr.jobList = allocateRes.jobList;
        self.runningInfo.quickArr.partyList = allocateRes.partyList;
        self.runningInfo.quickArr.survHmnNum = allocateRes.HmnNum;
        self.runningInfo.quickArr.survGstNum = allocateRes.GstNum;
        self.runningInfo.quickArr.survAlnNum = allocateRes.alnNum;
        self.runningInfo.words = initWordsRes;
        var jobExistArr = new Array(GameInfo.JobInfo.getJobTtlNum());
        for(var i = 0;i<allocateRes.jobList.length;i++){
           if(basicUtils.isEmpty(jobExistArr[allocateRes.jobList[i]])){
               jobExistArr[allocateRes.jobList[i]] = [allocateRes.jobList[i]];
           }else{
               jobExistArr[allocateRes.jobList[i]].push(i);
           }
        }
    ...
    this.initPlayerListContent = function(allocateRes){
        ...
        for(var i = 0;i<loop.length;i++){
            if(loop[i] === false) {
                ...
                self.runningInfo.playerListByPos[i] = new Player(self,myJob,myParty,i);
                nextPos++
            };
        }
    };
    ...
    this.quickQuery = {
        ...
        getTargetsByPos:function(posArr){
            if(!Array.isArray(posArr)){console.log("game发送推送目标参数错误");return;}
            res = [];
