...
exports.dawnLoop = function dawnLoop(game){
     ...
     * 过程：
     * 1、推送白天到来动画；
     * 2、等待ani秒，推送第num个白天到来消息
     * 3、间隔gap秒，刷新内存-黑夜行动导致的结果 → 可能有点播
     * 4、推送黎明信息获得队列
     * 5、受一定技能影响，刷新内存队列
     * 6、推送全局小心
     * */
    ...
    DL.step3A = function(game){
        ...
        if(endCheck.isEnd === true){
            ...
            //外星人在黑夜出局
            if(game.runningInfo.fleshInfo.ngtOutAlnPos !== 0){
                var aln = game.runningInfo.playerListByPos[game.runningInfo.fleshInfo.ngtOutAlnPos];
                game.runningInfo.fleshInfo.ngtOutAlnPos = 0;
                //已用先机
