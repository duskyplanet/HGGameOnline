...
exports.shootLoop = function shootLoop(game){
    ...
    SL.step1 = function(game){
        if(SL.tools.checkBless(game)){
            game.sender.onSkill("dayBless"); //TODO漫天祥云
            gb.loops.speakLoop(game);
        }else{
            if(game.runningInfo.fleshArr.lastNgtCanShoters.length > 0){
                game.sender.onInfo("wait","请等待出局者："+game.runningInfo.fleshArr.lastNgtCanShoters.join("、")+"号留遗言并开枪带人…"); //TODO漫天祥云
                setTimeout(function(){
                    game.sender.onJY(game.runningInfo.fleshArr.lastNgtCanShoters);
                    SL.step2();
                },timers.gap*2000);
            }else{
                gb.loops.speakLoop(game);
