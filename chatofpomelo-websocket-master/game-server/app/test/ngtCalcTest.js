...
var sur = [1,2,4,6,7,9,14,16];
var lastPos = -1;
var firstPos = 4;

console.log(firstPos);
setInterval(function(){
    var pos = findNextPos(sur,firstPos,lastPos);
    if(pos!==-1){
        lastPos = pos;
        console.log(pos);
    }else{
...
setTimeout(function(){
    hjcArr.deleteValue(sur,2);
},8000);

function findNextPos(sur,firstPos,lastPos){
    var speakArr = circleToHead(sur,firstPos);
    if(lastPos === -1){
        if(speakArr.length>1){
            return speakArr[1];
        } return lastPos;
    }
    for(var i = 1; i < speakArr.length; i++){
        if(lastPos === speakArr[i]){
            if(i===speakArr.length-1){
                return -1;
            }else{
                return speakArr[i+1];
            }
...
function circleToHead(arr,head){
    var headPos = hjcArr.firstPosition(arr,head);
    var returnArr = [];
    for(var i = headPos; i < arr.length; i++){
